...
const parseTrimTime = (value: unknown, defaultValue: number, fieldName: string): number => {
  ...
  if (!Number.isFinite(parsedValue)) {
    throw new HTTPException(400, { message: `${fieldName} must be a number` });
  }
...
.post("/", async (c) => {
  ...
  if (startTime < 0) {
    throw new HTTPException(400, { message: "startTime must be a non-negative number" });
  }

  if (endTime <= startTime) {
    throw new HTTPException(400, { message: "endTime must be greater than startTime" });
  }
